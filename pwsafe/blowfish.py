...
class Blowfish:
	...
	def __init__ (self, key):
		...
		for i in range (len (self.p_boxes)):
			val = (long (ord (key[index % key_len])) << 24) + \
			      (long (ord (key[(index + 1) % key_len])) << 16) + \
			      (long (ord (key[(index + 2) % key_len])) << 8) + \
			       long (ord (key[(index + 3) % key_len]))
	...
	def encrypt (self, data):
		...
		xl = long (ord (data[3])) | (long (ord (data[2])) << 8) | (long (ord (data[1])) << 16) | (long (ord (data[0])) << 24)
		xr = long (ord (data[7])) | (long (ord (data[6])) << 8) | (long (ord (data[5])) << 16) | (long (ord (data[4])) << 24)
	...
	def decrypt (self, data):
		...
		# Use big endianess since that's what everyone else uses
		cl = long (ord (data[3])) | (long (ord (data[2])) << 8) | (long (ord (data[1])) << 16) | (long (ord (data[0])) << 24)
		cr = long (ord (data[7])) | (long (ord (data[6])) << 8) | (long (ord (data[5])) << 16) | (long (ord (data[4])) << 24)
